<<<<<<< Updated upstream
function runall(result_type, save_name, result_name)
    ...
    for kkk = 1:5
        ...
        % train
        filename = sprintf('%s_%s',result_name, target_class);
        IOU = 0.5;
        type = '';
        fpath = sprintf('../results/%s/%s/%s/%s.txt', result_type, save_name, result_name, filename);
    ...
    %         end
        ...
        result_path = sprintf('../results/%s/%s/%s/results.txt', result_type, save_name, result_name);
...
for kkk = 1%:5
    ...
    %fpath = sprintf('../../results/test/comp4-27463_det_test_%s.txt',target_class);
    % train
    fpath = sprintf('../../results/train/%s_10000.txt',target_class);
